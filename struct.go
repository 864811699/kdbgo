...
func TimeZV(x []time.Time) *K {
	//localEpoch := time.Date(2000, time.January, 1, 0, 0, 0, 0, time.Local)
	//localEpoch := qEpoch.Add(time.Hour * 8)
	var diff []float64
	for _, inx := range x {
		indiff := inx.Add(time.Hour * 8).Sub(qEpoch).Seconds() / 86400
		diff = append(diff, indiff)
	}
...
func UnmarshalTable(t Table, v interface{}) ([]interface{}, error) {
	...
	for i := 0; i < t.Data[0].Len(); i++ {
		emptyelem := reflect.New(vv.Type())
		// emptyelem := reflect.New(vv.Type())
		err := UnmarshalDict(t.Index(i), emptyelem.Interface())
		if err != nil {
			fmt.Println("Failed to unmarshal dict", err)
			return nil, err
		}
		// fmt.Println("sv/kdbgo", emptyelem.Interface())
		// vv = reflect.Append(vv, reflect.Indirect(emptyelem).Elem())
		vvs = append(vvs, emptyelem.Interface())
